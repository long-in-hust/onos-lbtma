...
import static org.onosproject.pipelines.p4dpads.AppConstants.INGRESS_STATE_TABLE;
...
public class PipelinerImpl extends AbstractHandlerBehaviour implements Pipeliner {
    ...
    public void forward(ForwardingObjective obj) {
        ...
        // Check if the objective is a SPECIFIC rule to be translated
        if (obj.flag() != ForwardingObjective.Flag.SPECIFIC) {
            log.warn("Only SPECIFIC forwarding objectives are supported by this pipeconf");
            obj.context().ifPresent(c -> c.onError(obj, ObjectiveError.UNSUPPORTED));
            return; // Reject non-SPECIFIC objectives if you only support table programming
        }

        // Simply create an equivalent FlowRule for the state table.
        final FlowRule.Builder ruleBuilder = DefaultFlowRule.builder()
                .forTable(INGRESS_STATE_TABLE)
                .forDevice(deviceId)
